#!/usr/bin/env python3
"""Visualize one or more completed Atari runs using CORA metrics styling."""
...
from continual_rl.utils.cora_metrics import ATARI, TO_PLOT  # noqa: E402
from continual_rl.utils.metrics import Metrics  # noqa: E402


DEFAULT_RUN_DIRS = [
    REPO_ROOT / "srun_log" / "clear-atari_46490506",
]
DEFAULT_MAX_STEP = int(200e6)
...
class HtmlMetrics(Metrics):
    ...
    def write_overview_grid(self, d, x_range):
        ...
        rows, cols = self._experiment_data.get("grid_size", [2, 3])
        fig = make_subplots(
            rows=rows,
            cols=cols,
        ...
        grid_name = f"{rows}x{cols}"
        output_html = self._output_dir / f"{self._experiment_data['which_exp']}_overview_{grid_name}.html"
        fig.write_html(output_html)
        self.write_static_image(fig, self._output_dir / f"{self._experiment_data['which_exp']}_overview_{grid_name}.png")
...
def _display_name_for_run(run_root: Path) -> str:
    ...
    if run_name.startswith("clear-atari_"):
        return "CLEAR"
    if run_name.startswith("impala-atari_"):
        return "IMPALA"
    if run_name.startswith("pnc-atari_"):
        return "P&C"
    if run_name.startswith("ewc-atari_"):
        return "EWC"
    return run_name
...
def _model_style_for_run(run_root: Path, color_index: int) -> dict:
    ...
    if run_root.name.startswith("clear-atari_"):
        color = clear_colors[color_index % len(clear_colors)]
    elif run_root.name.startswith("impala-atari_"):
        color = impala_colors[color_index % len(impala_colors)]
    elif run_root.name.startswith("pnc-atari_"):
        color = "rgba(152, 67, 63, 1)"
    elif run_root.name.startswith("ewc-atari_"):
...
def build_experiment_data(run_roots: list[Path], tag_base: str, output_dir: Path, max_step: int | None) -> tuple[dict, list[str], dict]:
    ...
    tasks = adapt_tasks_for_available_tags(ATARI["tasks"], common_available_tags, tag_base)
    if not tasks:
        raise RuntimeError(
            f"No Atari tasks matched tag base '{tag_base}' across runs. "
            f"Available scalar tags include: {sorted(list(common_available_tags))[:20]}"
        )

    experiment_data = copy.deepcopy(TO_PLOT)
    experiment_data.update(copy.deepcopy(ATARI))
    models = {}
    metadata_runs = []
    ...
    experiment_data.update(
        exp_dir="",
        which_exp=f"atari_overlay_{tag_base}",
        models=models,
        tasks=tasks,
        num_tasks=len(tasks),
...
def default_output_dir_for_runs(run_roots: list[Path], tag_base: str) -> Path:
    ...
    return common_parent / "atari-comparison" / tag_base
...
<head>
  <meta charset="utf-8">
  <title>Atari Visualization Index</title>
  <style>
    :root {{
...
<body>
  <main>
    <div class="card">
      <h1>Atari Visualization Index</h1>
    ...
    <div class="card">
      <h2>Task Plots</h2>
      <p><a href="{metadata['overview_file']}">Open overview grid</a></p>
      <ul>
        {links_html}
      </ul>
    </div>
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="One or more root directories containing completed Atari runs.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Only plot and summarize points up to this global step. Default: 200000000.",
    )
...
def main() -> None:
    ...
    grid_rows, grid_cols = experiment_data.get("grid_size", [2, 3])
    metadata["overview_file"] = f"{metadata['which_exp']}_overview_{grid_rows}x{grid_cols}.html"
    metadata["task_tags"] = [
